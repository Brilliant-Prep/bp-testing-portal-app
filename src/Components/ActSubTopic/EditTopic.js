...
function EditTopic() {
  ...
  const getSinglTopic = async (id) => {
    const response = await axios.get(
      `${process.env.REACT_APP_BASE_URL}edit_actsubtopic/${id}`
    );
    if (response.status === 200) {
  ...
  const onSubmit = async (values) => {
    try {
      const { data } = await axios.post(
        `${process.env.REACT_APP_BASE_URL}update_actsubtopic/${id}`,
        {
          ...values,
          id: id, 
          formData: formData,
          ...
          <div class="col-12">
            <div class="row">
              ...
              <div class="col-6 text-right">
                <Link class="btn btn-outline-warning" to={`/actsubtopics`}>
                      Back to Sub Topics
                </Link>
              </div>
            </div>
