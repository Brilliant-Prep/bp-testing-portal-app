...
function EditTopic() {
  ...
  const getSinglTopic = async (id) => {
    const response = await axios.get(
      `${process.env.REACT_APP_BASE_URL}edit_acttopic/${id}`
    );
  ...
  const onSubmit = async (values) => {
    try {
      const { data } = await axios.post(
        `${process.env.REACT_APP_BASE_URL}update_acttopic/${id}`,
        {
          ...values,
          id: id, 
          formData: formData,
              ...
              <div class="col-12">
                <div class="row">
                  ...
                  <div class="col-6 text-right">
                      <Link to={`/acttopics`} class="btn btn-outline-primary"><i className="fa fa-arrow-left"></i> Back to Topics</Link>
                  </div>
                </div>
              </div>
              <br />
