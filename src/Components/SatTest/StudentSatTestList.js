...
function StudentSatTestList() {
  ...
  const unassignTest = async (testId) => { 
    if (window.confirm("Are you sure you want to Unassign Test?")) {
      const response = await axios.delete(
        `${process.env.REACT_APP_BASE_URL}unassignsatstest/${testId}/${id}`
      );
  ...
  const gettests = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_BASE_URL}getsatstests/${id}`
    );
  ...
  const handleSubmitAssign = async (e) => {
    ...
    if (tests.length === 0) {
      ...
      try {
        const { data } = await axios.post(`${process.env.REACT_APP_BASE_URL}assignteststudent`, dataSend, {
...
export default StudentSatTestList;
